"""
eropuru.com album scraper - downloads all photos from a model's page.

Usage:
    python scraper_eropuru.py [--model-id ID] [--output DIR]

Examples:
    python scraper_eropuru.py --model-id 41602
    python scraper_eropuru.py --model-id 41602 --output ./downloads

Images from img09.eropuru.com CDN. No auth required. No Cloudflare.
"""
...
BASE_URL = "https://www.eropuru.com"
HEADERS = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 "
                  "(KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
    "Referer": "https://www.eropuru.com/",
}
REQUEST_DELAY = 0.3
DOWNLOAD_DELAY = 0.15
MAX_RETRIES = 3
...
def send_feishu_summary(token, webhook_url, model, total_images, total_downloaded, sample_images=None):
    if not webhook_url:
        return
    lines = [f"**模特:** {model}"]
    lines.append(f"**来源:** eropuru.com")
    lines.append(f"**总图片:** {total_images} | **已下载:** {total_downloaded}")
    payload = {
        "msg_type": "interactive",
        "card": {
            "header": {"title": {"tag": "plain_text", "content": f"✅ eropuru 下载完成: {model}"}, "template": "green"},
            "elements": [{"tag": "div", "text": {"tag": "lark_md", "content": "\n".join(lines)}}],
        }
    }
    _feishu_post(webhook_url, payload)
...
def get_page_images(soup):
    ...
    for img in soup.select("img"):
        ...
        if not src or src.startswith("data:"):
            continue
        if "eropuru.com" not in src:
            continue
        if any(x in src for x in ["loading", "icon", "logo", "avatar"]):
            continue
...
def get_all_images(model_id):
    """Get all image URLs from all pages."""
    base_url = f"{BASE_URL}/zyoyu/{model_id}.html"
    all_images = []
    page = 1
    ...
    while True:
        ...
        print(f"  Page {page}: {len(images)} images (total: {len(all_images)})")
...
def main():
    ...
    print(f"[1/3] Fetching model page {args.model_id}...")
    images = get_all_images(args.model_id)
    if not images:
    ...
    dl_count = download_images(images, output, f"eropuru-{args.model_id}")
